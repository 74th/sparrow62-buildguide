...
- Lily58 のように、縦に揃ったキー配置(カラムスタッガード)であること。
- Lily58 のように、十分キーの数が多いこと。
- Lily58 のように、Pro Micro をキーの横に配置してキーボード自体を薄くすること。
...
- 薄さ、価格のやすさを優先し、ボトムプレートをキットに含まないこと。
- キースイッチを強く固定するトッププレートをキットに含まないこと。（ TODO: 本当に含まない？ ）
- ボトムプレート、トッププレートを固定するネジ等を含まないこと。
...
1. 先に PCB の片側にハンダをつける。
2. つけたハンダを溶かして、ダイオードを実装する。この時ダイオードを押して、PCB に未着させてからはんだごてを外す。
3. 反対側の足をはんだ付けする。
